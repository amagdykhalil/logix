        ...
        <div class="flex flex-wrap gap-4">
            @foreach ($options as $index => $option)
                <button type="button" @click="activeIndex = {{ $index }}"
                    ...
                    class="!w-[100px] !text-sm transition-colors h-[50px]  rounded-[8px] py-[5px] px-[15px] flex items-center justify-center gap-[10px] outline-none font-[400] lg:text-base leading-[100%] tracking-[0] text-center">
                    {{ $option['text'] }}
                </button>
            @endforeach
        </div>
